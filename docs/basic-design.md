...
現時点では[要件定義書](requirements.md)の◎項目のうち、タスク一覧・検索（読み取り）のみを実装済み。
作成・更新・削除・並び替えのAPIは今後実装予定。
...
- タスクの作成・更新・削除・並び替えAPI（ドラッグ&ドロップ対応）の設計
- カード詳細・追加モーダルのフロントエンド設計
